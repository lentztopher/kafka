...
class LogManagerTest {
  ...
  def testTimestampBasedLogSegmentCleanup() {
    ...
    // create a log
    val log = logManager.getOrCreateLog(new TopicPartition(name, 0), LogConfig.fromProps(logConfig.originals, initialProps))
    ...
    for(i <- 0 until numMessages) {
      val set = TestUtils.singletonRecords("test".getBytes(), timestamp = i + startTime)
