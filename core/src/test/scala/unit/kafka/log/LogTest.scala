...
class LogTest {
  ...
  def testNonSequentialAppend(): Unit = {
    ...
    try log.appendAsLeader(nextRecords, leaderEpoch = 0)
    finally log.close()
  }
  ...
  def testThatGarbageCollectingSegmentsDoesntChangeOffset() {
    for(messagesToAppend <- List(0, 1, 25)) {
      ...
      // cleanup the log
      log.delete()
      //TODO: Do we need to close before delete?
    }
  }
  ...
  def testAppendWithOutOfOrderOffsetsThrowsException() {
    ...
    try log.appendAsFollower(invalidRecord)
    finally log.close()
  }
  ...
  def testZombieCoordinatorFenced(): Unit = {
    ...
    try appendEndTxnMarkerAsLeader(log, pid, epoch, ControlRecordType.ABORT, coordinatorEpoch = 1)
    finally log.close()
  }
